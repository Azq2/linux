...
#define	PMB887X_INTC_FIQ_STAT			0x08
#define	PMB887X_INTC_FIQ_STAT_NUM		0xFF
#define	PMB887X_INTC_FIQ_STAT_NUM_SHIFT	0
#define	PMB887X_INTC_FIQ_STAT_UNREAD	BIT(16)
#define	PMB887X_INTC_FIQ_STAT_NOT_ACK	BIT(24)

#define	PMB887X_INTC_IRQ_STAT			0x0C
#define	PMB887X_INTC_IRQ_STAT_NUM		0xFF
#define	PMB887X_INTC_IRQ_STAT_NUM_SHIFT	0
#define	PMB887X_INTC_IRQ_STAT_UNREAD	BIT(16)
#define	PMB887X_INTC_IRQ_STAT_NOT_ACK	BIT(24)

#define	PMB887X_INTC_FIQ_ACK			0x10
#define	PMB887X_INTC_IRQ_ACK			0x14

#define	PMB887X_INTC_CURRENT_FIQ		0x18
#define	PMB887X_INTC_CURRENT_IRQ		0x1C

#define	PMB887X_INTC_CON(n)				(0x30 + ((n) * 0x4))
#define	PMB887X_INTC_CON_PRIORITY		0xFF
#define	PMB887X_INTC_CON_PRIORITY_SHIFT	0
#define	PMB887X_INTC_CON_FIQ			BIT(8)

#define PMB887X_INTC_NR_IRQ				170
...
static void __irq_entry pmb887x_intc_handler_irq(struct pt_regs *regs) {
	irq_hw_number_t hwirq = readl_relaxed(current_intc->irq_base + PMB887X_INTC_CURRENT_IRQ);
...
static void pmb887x_irqc_ack(struct irq_data *d) {
	...
	writel(1, priv->irq_base + PMB887X_INTC_IRQ_ACK);
}

static void pmb887x_irqc_mask(struct irq_data *d) {
	struct pmb887x_irqc_priv *priv = irq_data_get_irq_chip_data(d);
	writel(0, priv->irq_base + PMB887X_INTC_CON(d->hwirq));
}

static void pmb887x_irqc_unmask(struct irq_data *d) {
	struct pmb887x_irqc_priv *priv = irq_data_get_irq_chip_data(d);
	writel(1, priv->irq_base + PMB887X_INTC_CON(d->hwirq));
}
...
static int __init pmb887x_intc_init(struct device_node *node, struct device_node *parent) {
	...
	priv->irq_domain = irq_domain_add_linear(node, PMB887X_INTC_NR_IRQ, &pmb887x_irqc_ops, priv);
	if (!priv->irq_domain)
